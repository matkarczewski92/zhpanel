    ...
    <div class="modal-dialog modal-xl">
      <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <form action="{{ route('litters.gallery.store', [$litter->id]) }}" method="post" enctype="multipart/form-data">
            @csrf <!-- {{ csrf_field() }} -->
            ...
            @foreach ($litter->animalGallery as $photo)
                <div class="col">
                    <div class="card @if($photo->main_photo == 1) bg-light @endif">
                   <img src="{{$photo->url}}" class="card-img-top"alt="...">
                        <div class="card-body text-center">
                            <div class="btn-group" role="group" aria-label="Basic example">
                                <form action="{{ route('litters.gallery.destroy', [$photo->litter_id, $photo->id])}}" method="post">
                                    @method('DELETE')
                                    @csrf <!-- {{ csrf_field() }} -->
                                <button class="btn btn-danger" role="submit">Usuń</button>
                                </form>
                                @if(!$photo->main_photo == 1)
                                <a class="btn btn-success" href="{{ route('litters.gallery.edit', [$photo->litter_id, $photo->id])}}" role="button">Ustaw jako główne</a>
                                 @endif
                              </div>
                        </div>
                    </div>
