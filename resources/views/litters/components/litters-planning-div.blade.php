<div class="row mt-2">
    ...
    <div class="col-lg mt-1">
        <div class="card bg-dark photobg rounded-1 h-100">
            ...
            @foreach ($connection ?? '' as $con)
                <a href="{{ route('litters.show', $con->id)}}" data-toggle="tooltip" data-placement="top" title="{{$con->connection_date ?? $con->planned_connection_date }}">{{$con->litter_code}}</a>,
            @endforeach
            </div>
        </div>
    </div>
    <div class="col-lg mt-1">
        <div class="card bg-dark photobg rounded-1 h-100">
            ...
            @endphp
                <a href="{{ route('litters.show', $con->id)}}" data-toggle="tooltip" data-placement="top" title="{{$layingday}}">{{$con->litter_code}}</a>,
            @endforeach
            </div>
        </div>
    </div>
    <div class="col-lg mt-1">
        <div class="card bg-dark photobg rounded-1 h-100">
            ...
            @endphp
                <a href="{{ route('litters.show', $con->id)}}" data-toggle="tooltip" data-placement="top" title="{{$hatchlingday}}">{{$con->litter_code}}</a>,
            @endforeach
            </div>
        </div>
    </div>
